...
#while loop for including -v on command line for task 1
while [ $# -gt 0 ]; do        # $# for command line
case "$1" in
		-v )
		verbose="yes"
		;;
...
if [ -v interface ]; 
then
	echo "command line has so many interfaces , '$1' cannot be used"
exit 1
else
	interface="$1"
fi
...
everyinterface="$interface"
#command to find all the interfaces
if [ -z "$everyinterface" ]; then
everyinterface="$(ip r| grep -v default | awk '{print $3}' | sort | uniq)"
fi
for interface in $everyinterface; do

	[ "$verbose" = "yes" ] && echo "Report on interface(s): $interface"

	[ "$verbose" = "yes" ] && echo "Obtaining IPV4 address and name for interface $interface"
	# Find an address and hostname for the interface being summarized
	# we are assuming there is only one IPV4 address assigned to this interface
	ipv4_address=$(ip a s $interface|awk -F '[/ ]+' '/inet /{print $3}')
	ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')

	[ "$verbose" = "yes" ] && echo "Receiveing IPV4 network block info and name for interface $interface"
